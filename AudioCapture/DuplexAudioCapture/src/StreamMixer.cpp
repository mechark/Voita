...
std::vector<int16_t> StreamMixer::Impose(circular_buffer<int16_t>* iBuffer, circular_buffer<int16_t>* oBuffer, int debug) {
	size_t frameSize = max(oBuffer->getFrameSize(), iBuffer->getFrameSize());
	std::vector<int16_t> mixedFrame(frameSize);
	...
	for (size_t i = 0; i < frameSize; i++)
	{
		...
		mixedFrame[i] = static_cast<int16_t>(mixed);
	}
